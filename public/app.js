...
const OCR_CACHE_PREFIX = 'azal-notam-ocr-v9:';
...
function analyzeRoute(blocks, route) {
  ...
  for (const block of blocks) {
    ...
    const groupKey = block.noticeId || block.fingerprint || `${block.page}|${makeSnippet(block.text).slice(0, 260)}`;
    ...
    if (!existing) {
      grouped.set(groupKey, {
        ...
        snippet: block.displayText || makeDisplayText(block.text)
      });
      continue;
    }
    ...
    if (candidateText.length > existing.snippet.length) {
      existing.snippet = candidateText;
    }
  }
