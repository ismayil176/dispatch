...
const OCR_CACHE_PREFIX = 'azal-notam-ocr-v15:';
...
const ICAO_TOKEN_FRAGMENT = '[A-Z](?:\\s*[A-Z]){3}';
const NOTICE_CODE_FRAGMENT = '[A-Z]\\s*\\d{4}\\s*\\/\\s*\\d{2}';
const NOTICE_HEADER_RE = new RegExp(`\\b((?:${ICAO_TOKEN_FRAGMENT})(?:\\s+(?:${ICAO_TOKEN_FRAGMENT})){0,3})\\s+(${NOTICE_CODE_FRAGMENT})\\b`);
const NOTICE_START_RE = new RegExp(`^(?:[*\\s-]*(?:RECENT|RULE)[*\\s-]*)?(?:[-*]\\s*)?((?:${ICAO_TOKEN_FRAGMENT})(?:\\s+(?:${ICAO_TOKEN_FRAGMENT})){0,3})\\s+(${NOTICE_CODE_FRAGMENT})\\b`);
...
function isFuzzyEligibleToken(token) {
  return /^[A-Z]{5,}$/.test(String(token || ''));
}
...
function flexibleTokenPattern(token, options = {}) {
  const value = String(token || '').toUpperCase();
  const useFuzzy = options.fuzzy && isFuzzyEligibleToken(value);
  const maxGap = options.maxGap ?? (useFuzzy ? 2 : 1);
  return value
    .split('')
    .map((char) => (useFuzzy ? fuzzyCharPattern(char, true) : escapeRegExp(char)))
    .join(`[\\s\\-_/,:;()]{0,${maxGap}}`);
}

function makeTokenRegex(token, options = {}) {
  const pattern = flexibleTokenPattern(token, options);
  return new RegExp(`(^|[^A-Z0-9])(${pattern})(?=[^A-Z0-9]|$)`, 'i');
}

function makeLegRegex(leg, options = {}) {
  const from = flexibleTokenPattern(leg.from, options);
  const to = flexibleTokenPattern(leg.to, options);
  const airwayPart = leg.airways.length
    ? `${leg.airways.map((airway) => flexibleTokenPattern(airway, { ...options, fuzzy: false, maxGap: 1 })).join('[^A-Z0-9]{0,12}')}[^A-Z0-9]{0,18}`
    : '';
...
function stripNoticeHeaderFromDisplayText(text) {
  const value = makeDisplayText(text);
  if (!value) return value;
  ...
  const headerMatch = firstLine.match(NOTICE_HEADER_RE);

  if (headerMatch) {
    const trimmedFirstLine = firstLine.slice(headerMatch[0].length).trim();
    const remainingLines = [];
    if (trimmedFirstLine) remainingLines.push(trimmedFirstLine);
    remainingLines.push(...lines.slice(1));
...
function makeBodySearchText(text) {
  return normalizeForSearch(stripNoticeHeaderFromDisplayText(text));
}

function tokenAppearsInNoticeBody(token, text) {
  return makeTokenRegex(token, { fuzzy: false }).test(makeBodySearchText(text));
}

function segmentAppearsInNoticeBody(segment, text) {
  const [from, to] = String(segment || '').split('-').filter(Boolean);
  if (!from || !to) return false;
  return makeLegRegex({ from, to, airways: [] }, { fuzzy: false }).test(makeBodySearchText(text));
}
...
function collapseSpacedIcaoTokens(value) {
  const letters = String(value || '').toUpperCase().replace(/[^A-Z]/g, '');
  const parts = [];
  for (let index = 0; index + 3 < letters.length; index += 4) {
    parts.push(letters.slice(index, index + 4));
  }
  return parts.join(' ').trim();
}

function compactNoticeCode(value) {
  const normalized = normalizeForSearch(value)
    .replace(/\s*\/\s*/g, '/')
    .replace(/\s+/g, '')
    .trim();
  const match = normalized.match(/([A-Z][0-9]{4}\/[0-9]{2})/);
  return match ? match[1] : '';
}

function extractNoticeHeader(text) {
  const preview = normalizeSnippetText(text)
    .toUpperCase()
    .split('\n')
    .slice(0, 8)
    .join(' ')
  ...
  if (!preview) return '';
  const match = preview.match(NOTICE_HEADER_RE);
  if (!match) return '';

  const icao = collapseSpacedIcaoTokens(match[1]);
  const noticeCode = compactNoticeCode(match[2]);
  if (!icao || !noticeCode) return '';
  return `${icao} ${noticeCode}`.trim();
}

function extractNoticeCode(text) {
  const normalized = normalizeSnippetText(text).toUpperCase();
  const match = normalized.match(/\b([A-Z]\s*\d{4}\s*\/\s*\d{2})\b/);
  return match ? compactNoticeCode(match[1]) : '';
}

function extractNoticeId(text) {
  return extractNoticeHeader(text) || '';
}

function canonicalNoticeKey(noticeId) {
  const header = extractNoticeHeader(noticeId) || String(noticeId || '');
  return normalizeForSearch(header)
    .replace(/[OQ@]/g, '0')
    .replace(/[IL]/g, '1')
    .replace(/\s+/g, ' ')
    .trim();
...
function shouldMergeBlocks(current, next) {
  ...
  if (current.noticeCode && next.noticeCode && current.noticeCode === next.noticeCode) {
    return true;
  }

  if (!current.noticeId && current.noticeCode && next.noticeKey && next.noticeKey.includes(current.noticeCode)) {
    return true;
  }
  if (!next.noticeId && next.noticeCode && current.noticeKey && current.noticeKey.includes(next.noticeCode)) {
    return true;
  }

  const overlapLines = sharedLineCount(current.text, next.text);
  if (overlapLines >= 2) return true;
  if ((!current.noticeId || !next.noticeId) && overlapLines >= 1) return true;
  return false;
}
...
function mergeNeighborBlocks(blocks) {
  ...
  blocks.forEach((block) => {
    const current = {
      ...block,
      noticeId: block.noticeId || extractNoticeId(block.text),
      noticeCode: block.noticeCode || extractNoticeCode(block.text),
      noticeKey: block.noticeKey || canonicalNoticeKey(block.noticeId || block.text)
    };
    ...
    last.noticeCode = last.noticeCode || current.noticeCode;
...
function isSectionMarkerLine(line) {
  return /^\*?(?:RECENT|RULE)\*?$/i.test(String(line || '').trim());
}

function lineStartsNotice(line) {
  const value = normalizeSnippetText(line)
    .toUpperCase()
    .trim();
  if (!value || isSectionMarkerLine(value)) return false;
  return NOTICE_START_RE.test(value);
}

function flushParsedBlock(blocks, current, routeKeywords) {
  ...
  const noticeId = extractNoticeId(displayText);
  const noticeCode = extractNoticeCode(displayText);

  blocks.push({
    id: `b${blocks.length + 1}`,
    page: current.startPage || 1,
    startPage: current.startPage || 1,
    endPage: current.endPage || current.startPage || 1,
    text,
    displayText,
    search,
    noticeId,
    noticeCode,
    fingerprint: makeBlockFingerprint(text)
  });
}

function attachAnonymousBlocks(blocks) {
  if (!blocks.length) return blocks;
  const merged = [];
  ...
  for (const block of blocks) {
    const current = { ...block };
    const last = merged[merged.length - 1];
    if (!current.noticeId && last && (current.startPage || current.page || 1) <= (last.endPage || last.page || 1) + 1) {
      last.text = mergeNoticeTexts(last.text, current.text);
      last.displayText = makeDisplayText(last.text);
      last.search = normalizeForSearch(last.text);
      last.fingerprint = makeBlockFingerprint(last.text);
...
function extractBlocksFromPages(pages) {
  ...
  pages.forEach((pageText, index) => {
    ...
    const lines = normalizeSnippetText(pageText)
      .toUpperCase()
      .split('\n')
      .map((line) => line.trim())
      .filter(Boolean);
    ...
    lines.forEach((line) => {
      if (!line || /^NNNN$/i.test(line)) return;
      if (isSectionMarkerLine(line)) return;

      if (lineStartsNotice(line)) {
        flushParsedBlock(blocks, current, routeKeywords);
        current = startBlock(pageNo, line);
        return;
      }
      ...
      current.endPage = pageNo;
    });
  });

  flushParsedBlock(blocks, current, routeKeywords);
  return mergeNeighborBlocks(attachAnonymousBlocks(blocks));
}
...
function filterLabelsByBlockText(labels, blockText) {
  return labels.filter((label) => {
    if (!label) return false;
    if (label.includes('-')) return segmentAppearsInNoticeBody(label, blockText);
    return tokenAppearsInNoticeBody(label, blockText);
  });
}

function finalizeHitMatches(hit) {
  const finalAirways = uniqueValues((hit?.airways || []).filter((token) => tokenAppearsInNoticeBody(token, hit?.text || '')));
  const finalPoints = uniqueValues((hit?.points || []).filter((token) => tokenAppearsInNoticeBody(token, hit?.text || '')));
  const finalSegments = uniqueValues((hit?.segments || []).filter((segment) => segmentAppearsInNoticeBody(segment, hit?.text || '')));
...
function analyzeRoute(blocks, route) {
  ...
  const pointRegexes = pointTokens.map((token) => ({ token, exact: makeTokenRegex(token, { fuzzy: false }), fuzzy: makeTokenRegex(token, { fuzzy: true }) }));
  const airwayRegexes = route.airways.map((token) => ({ token, exact: makeTokenRegex(token, { fuzzy: false }) }));
  const legRegexes = route.legs.map((leg) => ({ leg, exact: makeLegRegex(leg, { fuzzy: false }), fuzzy: makeLegRegex(leg, { fuzzy: true }) }));
  ...
  for (const block of blocks) {
    const matchedLegsExact = legRegexes.filter((item) => item.exact.test(block.search));
    const matchedPointsExact = pointRegexes.filter((item) => item.exact.test(block.search));
    const matchedAirwaysExact = airwayRegexes.filter((item) => item.exact.test(block.search));

    let matchedLegs = matchedLegsExact;
    let matchedPoints = matchedPointsExact;
    let matchedAirways = matchedAirwaysExact;
    ...
    if (!positive) {
      const matchedLegsFuzzy = legRegexes.filter((item) => item.fuzzy.test(block.search));
      const matchedPointsFuzzy = pointRegexes.filter((item) => item.fuzzy.test(block.search));

      const fuzzyPositive =
        matchedLegsFuzzy.length > 0 ||
        matchedPointsFuzzy.length >= 2 ||
        (matchedPointsFuzzy.length >= 1 && routeKeywords.test(block.search));

      if (!fuzzyPositive) continue;

      matchedLegs = matchedLegsFuzzy;
      matchedPoints = matchedPointsFuzzy;
      matchedAirways = matchedAirwaysExact;
      positive = true;
    }

    const labels = filterLabelsByBlockText(hitLabelsFromMatches(matchedLegs, matchedAirways, matchedPoints), block.displayText || block.text);
    if (!labels.length) continue;

    const groupKey = block.noticeKey || block.noticeCode || canonicalNoticeKey(block.noticeId || block.text) || block.fingerprint || `${block.page}|${makeSnippet(block.text).slice(0, 260)}`;
  ...
  const items = Array.from(grouped.values())
    .map((item) => finalizeHitMatches(item))
...
function tokenIndexRanges(text, tokens) {
  ...
  tokens.forEach((token) => {
    const pattern = flexibleTokenPattern(token, { fuzzy: false, maxGap: 1 });
...
function renderHighlightedSnippet(text, hit) {
  ...
  const visibleBodyText = stripNoticeHeaderFromDisplayText(text) || bodyText;
  const tokens = tokensForHit(hit);
  const ranges = mergeIndexRanges(tokenIndexRanges(visibleBodyText, tokens), 0);
  ...
  ranges.forEach((range) => {
    html += escapeHtml(visibleBodyText.slice(cursor, range.start));
    html += `<mark class="snippet-hit">${escapeHtml(visibleBodyText.slice(range.start, range.end))}</mark>`;
    cursor = range.end;
  });
  html += escapeHtml(visibleBodyText.slice(cursor));
  return html;
}
