...
const OCR_CACHE_PREFIX = 'azal-notam-ocr-v14:';
...
function filterLabelsByBlockText(labels, blockSearch) {
  return labels.filter((label) => {
    if (!label) return false;
    if (label.includes('-')) return true;
    return makeTokenRegex(label, { fuzzy: false }).test(blockSearch);
  });
}

function finalizeHitMatches(hit) {
  const search = normalizeForSearch(hit?.text || '');
  const finalAirways = uniqueValues((hit?.airways || []).filter((token) => makeTokenRegex(token, { fuzzy: false }).test(search)));
  const finalPoints = uniqueValues((hit?.points || []).filter((token) => makeTokenRegex(token, { fuzzy: false }).test(search)));
  const finalSegments = uniqueValues((hit?.segments || []).filter((segment) => {
    const [from, to] = String(segment || '').split('-').filter(Boolean);
    if (!from || !to) return false;
    return makeLegRegex({ from, to, airways: [] }, { fuzzy: false }).test(search);
  }));
...
function analyzeRoute(blocks, route) {
  ...
  for (const block of blocks) {
    ...
    const labels = filterLabelsByBlockText(hitLabelsFromMatches(matchedLegs, matchedAirways, matchedPoints), block.search);
