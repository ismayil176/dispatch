...
const OCR_CACHE_PREFIX = 'azal-notam-ocr-v3:';
...
function extractBlocksFromPages(pages) {
  ...
  const routeKeywords = /(ATS RTE|AIRWAY|WAYPOINT|ENTRY\/EXIT|SEGMENT|SEGMENTS|UNAVBL|NOT AVBL|CLSD|CLOSED|RESTRICTED|AUP|TSA|TRA|DUE TO OPS|NOT AVAILABLE|ADVISED TO USE)/;
...
function analyzeRoute(blocks, route) {
  ...
  const routeKeywords = /(ATS RTE|AIRWAY|WAYPOINT|ENTRY\/EXIT|SEGMENT|SEGMENTS|UNAVBL|NOT AVBL|CLSD|CLOSED|RESTRICTED|PROHIBITED|AUP|TSA|TRA|ACT|DUE TO OPS|NOT AVAILABLE|ADVISED TO USE)/;
...
async function parseFlightPlanPdf(file) {
  ...
  setStatus('Surətli metn cixarilir...', 10);
  ...
  const mergedPages = ocrPages.map((ocrText, index) => {
    ...
    if (quickText.replace(/[^A-Z]/gi, '').length > 250) {
