...
const OCR_CACHE_PREFIX = 'azal-notam-ocr-v8:';
...
function makeSnippet(text) {
  const lines = normalizeSnippetText(text).toUpperCase()
    ...
    .map((line) => line.trim())
    .filter(Boolean)
    .filter((line) => !/^\/\/ END PART \d+ OF \d+ \/\/$/i.test(line));

  return lines.slice(0, 10).join('\n').slice(0, 700);
}

function extractNoticeId(text) {
  const match = normalizeForSearch(text).match(/\b([A-Z]{4}\s+[A-Z]\d{4}\/\d{2})\b/);
  return match ? match[1] : '';
}

function extractBlocksFromPages(pages) {
  const blocks = [];
  const boundaryRe = /(?=\*RECENT\*|\*RULE\*|(?:^|\n)(?:[A-Z]{4}(?:\s+[A-Z]{4}){0,3})\s+[A-Z]\d{4}\/\d{2})/gm;
  ...
  pages.forEach((pageText, index) => {
    const normalized = normalizeSnippetText(pageText);
...
function analyzeRoute(blocks, route) {
  ...
  for (const block of blocks) {
    ...
    const dedupeHead = makeSnippet(block.text).slice(0, 220);
    const key = [block.noticeId || `P${block.page}`, labels.join('|'), dedupeHead].join('|');
  ...
  const dedupedByNotice = [];
  const noticeSeen = new Set();
  items.sort((a, b) => a.page - b.page || (a.noticeId || '').localeCompare(b.noticeId || ''));
  for (const item of items) {
    const noticeKey = item.noticeId ? `${item.noticeId}|${item.labels.join('|')}` : '';
    if (noticeKey && noticeSeen.has(noticeKey)) continue;
    if (noticeKey) noticeSeen.add(noticeKey);
    dedupedByNotice.push(item);
  }

  return {
    hasNotam: dedupedByNotice.length > 0,
    items: dedupedByNotice,
    matchedRouteItems: uniqueValues(dedupedByNotice.flatMap((item) => item.labels))
  };
}
...
async function parseFlightPlanPdf(file) {
  ...
  const mergedPages = ocrPages.map((ocrText) => normalizeSnippetText(ocrText));
...
async function countriesAlongSegment(from, to) {
  ...
  for (let step = 0; step <= steps; step += 1) {
    ...
    if (!country?.code || country.code === lastCode) continue;
...
async function resolveCountries(route) {
  ...
  for (let i = 0; i < candidatesByIndex.length; i += 1) {
    const exact = candidatesByIndex[i].filter((item) => item.code);
    if (exact.length === 1) {
      chosen[i] = exact[0];
    }
  }
  ...
  for (let pass = 0; pass < 4; pass += 1) {
    for (let i = 0; i < candidatesByIndex.length; i += 1) {
      ...
      const ranked = candidates
        .map((candidate) => {
          let score = distanceKm(left, candidate) + distanceKm(candidate, right);
          if (candidate.lat == null || candidate.lon == null) score += 250;
          if (candidate.source === 'override') score -= 30;
          if (candidate.source === 'icao-prefix') score -= 20;
          if (left?.code && candidate.code === left.code) score -= 500;
          if (right?.code && candidate.code === right.code) score -= 500;
