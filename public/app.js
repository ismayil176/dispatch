...
const OCR_CACHE_PREFIX = 'azal-notam-ocr-v10:';
...
function analyzeRoute(blocks, route) {
  ...
  for (const block of blocks) {
    ...
    if (candidateText.length > (existing.text || '').length) {
      existing.text = candidateText;
    }
  }
...
function renderHighlightedSnippet(text, hit) {
  const snippet = makeFocusedSnippet(text, hit);
  const tokens = tokensForHit(hit);
  const ranges = mergeIndexRanges(tokenIndexRanges(snippet, tokens), 0);
  ...
  ranges.forEach((range) => {
    html += escapeHtml(snippet.slice(cursor, range.start));
    html += `<mark class="snippet-hit">${escapeHtml(snippet.slice(range.start, range.end))}</mark>`;
    cursor = range.end;
  });
